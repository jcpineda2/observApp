...
<section class="px-4 pt-6 pb-4">
    <div class="flex items-start justify-between gap-4">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-900 leading-tight">Turismo Receptivo</h2>
            <p class="text-slate-500 text-sm mt-1">Visitantes internacionales y flujo de entrada.</p>
        </div>
...
<section class="px-4 pb-6">
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-xl border border-slate-100 p-4">
            ...
            <p class="text-2xl font-bold mt-1">{{ number_format((int)$totals->tourist_arrivals) }}</p>
        </div>

        <div class="bg-white rounded-xl border border-slate-100 p-4">
            <p class="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Excursionistas</p>
            <p class="text-2xl font-bold mt-1">{{ number_format((int)$totals->excursionist_arrivals) }}</p>
        </div>

        <div class="bg-white rounded-xl border border-slate-100 p-4">
            <p class="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Divisas</p>
            <p class="text-xl font-bold mt-1">
                {{ number_format((float)$totals->foreign_exchange_revenue, 0, ',', '.') }}
            </p>
        </div>

        <div class="bg-white rounded-xl border border-slate-100 p-4">
            <p class="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Estadía Prom.</p>
            <p class="text-xl font-bold mt-1">{{ number_format((float)$totals->average_stay, 2) }}</p>
        </div>
    </div>

    <div class="mt-4 bg-white rounded-xl border border-slate-100 p-4">
        <h3 class="text-sm font-bold text-slate-900">Top 5 países de residencia</h3>
