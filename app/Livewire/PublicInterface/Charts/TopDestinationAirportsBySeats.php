...
class TopDestinationAirportsBySeats extends Component
{
    ...
    private function buildChart(): void
    {
        ...
        $rows = AirConnectivityRoute::query()
            ->leftJoin('airports as a', 'a.id', '=', 'air_connectivity_routes.destination_airport_id')
            ->where('air_connectivity_routes.year_id', $this->year)
            ->when($this->month, fn ($q) => $q->where('air_connectivity_routes.month_id', $this->month))
            ->selectRaw('COALESCE(a.name, a.name, "Destino") as airport, SUM(air_connectivity_routes.seats_count) as seats')
            ->groupBy('airport')
            ->orderByDesc('seats')
            ->limit($this->limit)
            ->get();
    ...
    {
        return view('livewire.public-interface.charts.top-destination-airports-by-seats');
    }
}
