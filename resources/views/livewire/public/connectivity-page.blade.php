<div class="public-section public-section-spacing">
    ...
    <div class="public-two-column-grid">
        <div wire:key="connectivity-flights-seats-by-month-{{ $year ?? 'null' }}">
            <x-chart.card title="Vuelos y asientos por mes" subtitle="Serie mensual de conectividad aérea"
                :chart-id="'connectivity-flights-seats-by-month-' . ($year ?? 'null')" type="line" bodyClass="public-chart-standard" :labels="$flightsSeatsByMonth['labels'] ?? []"
                :datasets="[
                    [
                        'label' => 'Vuelos',
        ...
        <div wire:key="connectivity-top-airlines-by-seats-{{ $year ?? 'null' }}-{{ $month ?? 'all' }}">
            <x-chart.card title="Top aerolíneas por asientos" subtitle="Capacidad ofertada por aerolínea"
                :chart-id="'connectivity-top-airlines-by-seats-' . ($year ?? 'null') . '-' . ($month ?? 'all')" type="bar" bodyClass="public-chart-standard" :labels="$topAirlinesBySeats['labels'] ?? []"
                :datasets="[
                    [
                        'label' => 'Asientos',
    ...
    <div class="public-two-column-grid">
        <div wire:key="connectivity-top-origin-airports-{{ $year ?? 'null' }}-{{ $month ?? 'all' }}">
            <x-chart.card title="Top aeropuertos origen" subtitle="Aeropuertos de salida con mayor capacidad"
                :chart-id="'connectivity-top-origin-airports-' . ($year ?? 'null') . '-' . ($month ?? 'all')" type="bar" bodyClass="public-chart-standard" :labels="$topOriginAirports['labels'] ?? []"
                :datasets="[
                    [
                        'label' => 'Asientos',
        ...
        <div wire:key="connectivity-top-destination-airports-{{ $year ?? 'null' }}-{{ $month ?? 'all' }}">
            <x-chart.card title="Top aeropuertos destino" subtitle="Aeropuertos de llegada con mayor capacidad"
                :chart-id="'connectivity-top-destination-airports-' . ($year ?? 'null') . '-' . ($month ?? 'all')" type="bar" bodyClass="public-chart-standard" :labels="$topDestinationAirports['labels'] ?? []"
                :datasets="[
                    [
                        'label' => 'Asientos',
    ...
    <div class="public-two-column-grid">
        <div wire:key="connectivity-destinations-by-country-{{ $year ?? 'null' }}-{{ $month ?? 'all' }}">
            <x-chart.card title="Destinos por país" subtitle="Segmentación por país de destino" :chart-id="'connectivity-destinations-by-country-' . ($year ?? 'null') . '-' . ($month ?? 'all')"
                type="doughnut" bodyClass="public-chart-standard" :labels="$destinationsByCountry['labels'] ?? []" :datasets="[
                    [
                        'label' => 'Destinos',
        ...
        <div wire:key="connectivity-destinations-by-city-{{ $year ?? 'null' }}-{{ $month ?? 'all' }}">
            <x-chart.card title="Destinos por ciudad" subtitle="Top ciudades conectadas" :chart-id="'connectivity-destinations-by-city-' . ($year ?? 'null') . '-' . ($month ?? 'all')"
                type="bar" bodyClass="public-chart-standard" :labels="$destinationsByCity['labels'] ?? []" :datasets="[
                    [
                        'label' => 'Destinos',
